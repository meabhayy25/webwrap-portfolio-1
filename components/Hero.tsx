...
const Hero: React.FC = () => {
  ...
  // Softer spring for less jittery movement
  ...
  const iconTranslateX = useTransform(mouseXSpring, [-0.5, 0.5], ["-20px", "20px"]);
  const iconTranslateY = useTransform(mouseYSpring, [-0.5, 0.5], ["-20px", "20px"]);

  // Parallax for Background Giant Logo
  const bgLogoX = useTransform(mouseXSpring, [-0.5, 0.5], ["30px", "-30px"]);
  const bgLogoY = useTransform(mouseYSpring, [-0.5, 0.5], ["30px", "-30px"]);

  const handleMouseMove = (e: React.MouseEvent) => {
    // Only process on larger screens to save mobile battery/performance
  ...
  return (
    <section 
      ...
      className="relative pt-36 pb-20 md:pt-48 md:pb-32 overflow-hidden flex flex-col items-center text-center perspective-[2000px] min-h-screen justify-center will-change-transform"
    >
      {/* Dynamic Background Grid - Static CSS is faster than SVG for grids if simple, but here masking is used. 
          Optimized by adding will-change: transform to main container */}
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(to_right,#80808008_1px,transparent_1px),linear-gradient(to_bottom,#80808008_1px,transparent_1px)] bg-[size:32px_32px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] pointer-events-none"></div>
      
      {/* GIANT BACKGROUND LOGO - Optimized Opacity and Animation */}
      <div className="absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 pointer-events-none select-none">
        <motion.div
           ...
           className="opacity-[0.10] will-change-transform"
        >
          <svg viewBox="0 0 40 40" className="w-[120vw] h-[120vw] md:w-[1000px] md:h-[1000px] overflow-visible">
            ...
            {/* Reduced stroke widths for cleaner rendering */}
            <motion.path 
      ...
      >
        ...
        >
          <button 
            ...
            className="group relative h-14 px-10 rounded-full bg-white text-black font-bold text-lg hover:bg-zinc-100 transition-all w-full sm:w-auto overflow-hidden shadow-[0_0_40px_rgba(255,255,255,0.3)] hover:shadow-[0_0_60px_rgba(255,255,255,0.5)] active:transform active:scale-95"
          >
        ...
        >
            {/* New Floating Icons - Hidden on Mobile for performance */}
            <motion.div 
              style={{ x: iconTranslateX, y: iconTranslateY }}
              className="absolute -top-12 -right-12 z-30 hidden lg:block"
            >
              <div className="w-16 h-16 rounded-2xl bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 flex items-center justify-center shadow-2xl">
            ...
            >
              <div className="w-14 h-14 rounded-2xl bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 flex items-center justify-center shadow-2xl">
                <Sparkles className="text-purple-500" size={28} />
              </div>
            </motion.div>

            <div className="relative rounded-2xl bg-[#080808] border border-white/10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden aspect-[16/10] md:aspect-[21/9] group transform-style-3d">
               {/* Reflection/Shine */}
               <div className="absolute inset-0 bg-gradient-to-tr from-white/5 via-transparent to-transparent z-20 pointer-events-none mix-blend-overlay"></div>
                ...
                <div className="relative h-full bg-[#050505] flex p-1">
                   ...
                   >
                       <div className="w-10 h-10 rounded-xl bg-blue-600/10 mb-4 flex items-center justify-center border border-blue-500/20">
                       ...
                       >
                          <div className="flex items-center gap-4">
                             <div className="w-12 h-12 rounded-xl bg-green-500/10 flex items-center justify-center text-green-400 relative border border-green-500/20">
                                ...
                                <div className="absolute inset-0 rounded-xl border border-green-500/30 opacity-50"></div>
                             </div>
                             <div>
        ...
        {/* Infinite Scroll Marquee - Optimized with hardware acceleration */}
